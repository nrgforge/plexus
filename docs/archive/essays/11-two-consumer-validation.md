...
This essay describes a spike experiment that validated the two-consumer cross-dimensional architecture by simulating Trellis (writing fragments) and Carrel (provenance marks) contributing to a shared context with overlapping tag vocabulary. The research confirmed the core thesis: two independent consumers producing to the same context with shared tag normalization automatically generate cross-dimensional connections — from a research mark to writing fragments through shared concept nodes — that neither consumer could produce alone. The key finding is that tags are the shared language enabling convergence, and that TagConceptBridger's normalization (strip `#`, lowercase, prepend `concept:`) is sufficient to connect informally tagged fragments to formally tagged research annotations. The essay also identifies a concrete gap: both adapters return empty outbound event vectors, meaning consumers cannot discover what the pipeline learned without polling the graph, and proposes a distinction between confirmation events (ACKs) and discovery events (where Plexus adds value).
