...
**Started:** 2026-04-17
**Current phase:** BUILD (in progress) — DISCOVER update complete 2026-04-17; MODEL light-touch pass complete 2026-04-20; DECIDE complete 2026-04-21; ARCHITECT light-touch pass complete 2026-04-22; WP-A complete 2026-04-22 (`f82bd76`); WP-C complete 2026-04-22 (`4c028aa`)
**Artifact base:** ./docs/
...
| BUILD | ▶ In progress | WP-A ✅ `f82bd76` (ADR-039 `created_at` contract, D-01..D-04); WP-C ✅ `4c028aa` (D-05 DiscoveryGap trigger-source contract docstring + D-07 stale ADR-009 reference drop in `src/graph/node.rs`); WP-B, WP-D, WP-E pending | WP-A coordinated four-site `fix:` landed: ContentAdapter / ExtractionCoordinator / DeclarativeAdapter write ISO-8601 UTC to `node.properties["created_at"]`; TemporalProximityEnrichment parses RFC-3339 with graceful degradation. WP-C single `docs:` commit: DiscoveryGap module doc now carries ADR-040 trigger-source contract (activation paths, silent-idle-by-design framing, source-agnostic trigger); `src/graph/node.rs` cites ADR-042 for dimension extensibility instead of the stale ADR-009 reference. Next: WP-B (`resolve_dimension` + `validate_spec` refactor, ADR-042, D-06) — the cycle's highest-consequence structural violation. Then WP-D (onboarding deliverables — ADR-038 reframing hinges on its quality bar). WP-E optional. |
