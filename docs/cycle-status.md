...
**Started:** 2026-04-17
**Current phase:** BUILD (in progress) — DISCOVER update complete 2026-04-17; MODEL light-touch pass complete 2026-04-20; DECIDE complete 2026-04-21; ARCHITECT light-touch pass complete 2026-04-22; WP-A complete 2026-04-22 (`f82bd76`); WP-C complete 2026-04-22 (`4c028aa`); WP-B complete 2026-04-22 (`2cc25ee`); WP-D complete 2026-04-23 (pending commit)
**Artifact base:** ./docs/
...
| BUILD | ▶ In progress | WP-A ✅ `f82bd76`; WP-C ✅ `4c028aa`; WP-B ✅ `2cc25ee`; WP-D ✅ (pending commit, 2026-04-23); WP-E pending | WP-A coordinated four-site `fix:` landed: ContentAdapter / ExtractionCoordinator / DeclarativeAdapter write ISO-8601 UTC to `node.properties["created_at"]`; TemporalProximityEnrichment parses RFC-3339 with graceful degradation. WP-C single `docs:` commit. WP-B: `validate_dimension_syntax` + load-time walk of CreateNode/CreateEdge/ForEach primitives per Invariant 60. **WP-D 2026-04-23 — ADR-038 onboarding deliverables landed with substance:** worked-example spec at `examples/specs/embedding-activation.yaml` (declarative adapter + `ensemble: embedding-similarity`) with companion Python script at `.llm-orc/scripts/embedding/embedding-similarity.py` and ensemble config at `.llm-orc/ensembles/embedding-similarity.yaml`; two fixture corpora at `test-corpora/collective-intelligence/` (8 arXiv abstracts, collective-intelligence subfields) and `test-corpora/public-domain-stories/` (6 PG short stories, motif-diverse) with transparent CURATION documentation and falsifiability invitations; T12 acceptance test in `tests/acceptance/mcp_matrix_llm_orc.rs` exercises the worked example end-to-end via real Ollama subprocess (2.27s, nomic-embed-text); README rewrite names the lean baseline honestly (2 active by default, DiscoveryGap idle, EmbeddingSimilarity absent), the two activation paths, and capability-loss explicitly (no in-process embedding, no semantic extraction without llm-orc); new spec-author guide at `docs/references/spec-author-guide.md` covers dimension choice, lens grammar convention (composition-shape grounding + phenomenology-as-hypothesis), ensemble integration, minimum-useful-spec; `CreateNodePrimitive` + ContentAdapter + ExtractionCoordinator module docs name shipped-adapter dimension conventions inline for grep-discoverability. **Tautology threshold crossed:** within-corpus similarity 0.72+ (strongest: `swarms-phase-transitions ↔ idle-ants-role` at 0.855; `desirees-baby ↔ gift-of-the-magi` at 0.897); cross-corpus similarity all below threshold; legible within-abstract sub-clusters (biological pair `honeybee ↔ idle-ants` at 0.816; human-crowd pair `mosh-pit ↔ autonomous-vehicles` at 0.805). 444 lib + 86 acceptance + 1 doc = 531 tests green (+1 T12). Next: WP-E optional. |
