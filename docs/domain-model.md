...
Extracted from: ADR-001, semantic-adapters.md, semantic-adapters-design.md, PAPER.md, SPIKE-OUTCOME.md, Essay 07 (first adapter pair), Essay 08 (runtime architecture), Essay 09 (public surface), Essay 12 (provenance as epistemological infrastructure), Essay 13 (two-consumer validation revisited), Essay 17 (storage architecture), Essay 18 (phased extraction architecture). Discovery gap concepts from design discussion (2026-02-14).
...
| **Declarative enrichment** | A YAML-specified enrichment for bridging patterns not covered by parameterized built-ins. Uses `match` (filter events), `find_nodes` (query context), `guard` (idempotency check), `emit` (create edges/nodes). Still graph-wide and reactive — runs in the enrichment loop. Tier 1 in the enrichment tier model. | scripted enrichment |
| **Graph analysis** | Network science algorithms (PageRank, community detection, betweenness centrality, HITS, label propagation) that operate on global graph topology. Distinct from enrichment: runs as llm-orc ensembles with script agents, triggered on-demand or by threshold, NOT in the per-emission enrichment loop. Results are property updates on existing nodes, applied back through `ingest()`. Tier 2 in the enrichment tier model. | batch enrichment (misleading — graph analysis is not an enrichment) |
...
| **analyze graph** | llm-orc ensemble | Context | Run network science algorithms on a context's graph topology. Export graph → run algorithms (PageRank, community detection in parallel) → apply property updates back through `ingest()`. Triggered on-demand (`plexus analyze`) or by threshold. Not part of the enrichment loop. |
...
- **Graph analysis** runs outside the **enrichment loop**, results entering via **ingest()**
- **Enrichments** fire after each phase's **emission** — the graph enriches incrementally as phases complete, no "wait for all phases" barrier
...
- **Latent evidence** enters via **ingest()** using the **graph analysis** pattern (ADR-023) — batch-computed embeddings, stored as node properties or emitted as `similar_to` edges above a threshold
...
49. Graph analysis results enter the graph via `ingest()`, not through the enrichment loop. Graph analysis is not reactive per-emission — it is triggered on-demand or by threshold. The enrichment loop (Invariant 36) governs reactive enrichments; graph analysis operates outside it.
...
### 13. "Enrichment" ≠ "Graph analysis"
Both improve the graph beyond what adapters directly emit, but they have fundamentally different execution models. An **enrichment** is reactive — it fires in the enrichment loop after every emission, scans the full context snapshot, and terminates via idempotency. **Graph analysis** runs on demand outside the enrichment loop — triggered by a user command or threshold, executed as an llm-orc ensemble with script agents, results re-entering via `ingest()`. Enrichments operate at emission cadence; graph analysis operates at batch cadence. Never call graph analysis an "enrichment" — it does not participate in the enrichment loop and does not terminate via idempotency (Invariant 49).
...
### Introduced by Essay 18 (Phased Extraction Architecture)

**12. Enrichment quiescence under broader trigger scope.**
Invariant 50 establishes that enrichments are structure-aware, not type-aware — CoOccurrenceEnrichment should fire for any node connected via the configured relationship, regardless of source adapter. The current implementation implicitly assumes fragment source nodes. Generalizing the trigger scope requires verifying that the enrichment loop still reaches quiescence (Invariant 36) with the broader scope. Specifically: does CoOccurrenceEnrichment's idempotency check (`count / max_count` normalization) remain stable when source nodes come from heterogeneous adapters with different emission patterns?

**13. Declarative enrichment termination guarantees.**

Tier 1 declarative enrichments (YAML-specified, graph-wide, reactive) must terminate via idempotency per Invariant 36. But declarative specs could express patterns that don't converge — e.g., an enrichment that creates nodes which trigger itself. What constraints on the declarative enrichment primitives ensure termination? The `guard` primitive (idempotency check) is necessary but is it sufficient? May need a formal analysis or conservative restrictions during the `/rdd-decide` phase.
...
Latent evidence (embedding-derived similarity) would enter the graph via the graph analysis pattern (ADR-023): batch-computed embeddings stored as node properties, optionally producing `similar_to` edges above a similarity threshold. The discovery gap between structural and latent evidence is informative because the two layers are independently sourced. Open sub-questions:

- **Embedding production.** Which embedding models for which domains? A general-purpose text embedding (e.g., all-MiniLM-L6-v2) covers Trellis and Carrel; a movement-quality embedding informed by Laban effort dimensions covers EDDI. Should embedding be a graph analysis ensemble (whole-graph batch), a per-file extraction phase, or both?
- **Discovery gap computation.** Should the gap be surfaced as a query-time computation (compare structural and latent edges on demand), an enrichment (react to new `similar_to` edges and flag gaps), or a graph analysis (batch sweep for structurally-disconnected but latently-similar pairs)? Each has different cost and freshness tradeoffs.
...
Does not block extraction architecture or graph analysis work. Shapes the future design of graph analysis ensembles and may introduce a new dimension (latent) alongside structure, semantic, and provenance.
