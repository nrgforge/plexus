...
| **SemanticAdapter** | A Rust adapter for Phase 3 semantic extraction. Invokes an llm-orc ensemble, parses multi-agent results (SpaCy NER, themes, concepts/relationships), produces emissions with per-agent contribution keys and provenance trail (chain + marks + contains). Distinct from DeclarativeAdapter. | Phase 3 adapter, ensemble adapter |
