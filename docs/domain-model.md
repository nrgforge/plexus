...
| **Core enrichment** | A Rust-native enrichment implementing a general graph algorithm fundamental to Plexus's discovery capabilities. Fast (microseconds to low milliseconds), reactive (fires in the enrichment loop after every emission), parameterizable, and domain-agnostic. Five core enrichments: TagConceptBridger (provenance → semantic bridging), CoOccurrenceEnrichment (shared-source patterns), DiscoveryGapEnrichment (latent-structural disagreement), TemporalProximityEnrichment (timestamp-based co-occurrence), EmbeddingSimilarityEnrichment (latent evidence via vector similarity, ADR-026). These are not optional plugins — they define what kind of knowledge graph engine Plexus is. | built-in enrichment, Tier 0 enrichment |
