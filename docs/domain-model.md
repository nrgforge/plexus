...
| **Tag-to-concept bridging** | Automatic creation of `references` edges between marks and concept nodes when tags match concept node IDs in the same context. Previously implemented as a core enrichment (`TagConceptBridger`). *(Removed: TagConceptBridger was a domain-specific enrichment; domains needing tag bridging implement their own adapter or declare `tag_concept_bridger` in their adapter spec's `enrichments:` section.)* Bridges bidirectionally: new marks to existing concepts and new concepts to existing marks. The connection between what a user annotates and what the adapter layer discovered. | auto-bridging |
...
| **Integration** | A registered bundle of adapter + enrichments for a specific consumer. Example: `register_integration("trellis", adapter: FragmentAdapter, enrichments: [CoOccurrenceEnrichment])`. Enrichments shared across integrations are deduplicated by `id()`. The consumer interacts with a high-level API that hides the internal decomposition. | registration, binding |
...
| **Core enrichment** | A Rust-native enrichment implementing a general graph algorithm fundamental to Plexus's discovery capabilities. Fast (microseconds to low milliseconds), reactive (fires in the enrichment loop after every emission), parameterizable, and domain-agnostic. Four core enrichments: CoOccurrenceEnrichment (shared-source patterns), DiscoveryGapEnrichment (latent-structural disagreement), TemporalProximityEnrichment (timestamp-based co-occurrence), EmbeddingSimilarityEnrichment (latent evidence via vector similarity, ADR-026). *(TagConceptBridger was removed — tag bridging is domain-specific; domains that need it declare it in their adapter spec's `enrichments:` section.)* These are not optional plugins — they define what kind of knowledge graph engine Plexus is. | built-in enrichment, Tier 0 enrichment |
...
| **bridge** | *(Removed: TagConceptBridger was removed — tag bridging is domain-specific. Domains that need this behavior declare `tag_concept_bridger` in their adapter spec's `enrichments:` section.)* | Mark ↔ Concept | Previously: automatically create `references` cross-dimensional edges between marks and concept nodes whose IDs match (after tag format normalization). |
...
- *(TagConceptBridger was removed — tag bridging is domain-specific. Domains needing this behavior declare it in their adapter spec. When active, it creates `references` edges from adapter-produced marks to matching concept nodes, making every concept's origin graph-traversable: concept ← `references` ← mark ← `contains` ← chain.)*
...
27. *(No longer applies as a default invariant — TagConceptBridger was removed from the default pipeline. Tag-to-concept bridging is opt-in: domains that use it declare `tag_concept_bridger` in their adapter spec's `enrichments:` section. When active, the behavior is: the enrichment detects new concept nodes, finds marks with matching tags, and creates cross-dimensional `references` edges. Because the enrichment runs on every emission round, it bridges in both directions: a new mark bridges to existing concepts, and a new concept bridges to existing marks.)*
