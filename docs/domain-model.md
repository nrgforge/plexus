...
| **route** | IngestPipeline | Adapter(s) | Direct incoming input to all adapters whose `input_kind()` matches. Fan-out: multiple adapters can declare the same input kind. Implemented inline in `IngestPipeline.ingest()` via `classify_input`. |
...
| **bridge** | *(Removed: TagConceptBridger was fully removed — tag bridging is domain-specific. Domains that need this behavior implement their own adapter.)* | Mark ↔ Concept | Previously: automatically create `references` cross-dimensional edges between marks and concept nodes whose IDs match (after tag format normalization). |
...
| **extract** | Extraction coordinator | Source file | Process a source file through extraction phases. Registration runs synchronously within a single `ingest()` call; structural analysis and semantic extraction are spawned as sequential background tasks that call `ingest()` again. Each phase produces its own emission with its own adapter ID. |
...
| **analyze structure** | Structural module | Source file | Process a source file through heuristic structural analysis. Each module reads the file content and produces a `StructuralOutput` — what that contains depends on the modality (sections and vocabulary for structured text, function signatures for code, nothing for opaque formats). The module also emits graph mutations (concept nodes from extracted terms, structural edges) with its own adapter ID via `ModuleEmission`. Multiple modules may analyze the same file (fan-out by MIME type). A module that finds no useful structure returns an empty output — this is normal, not an error. |
| **dispatch modules** | Extraction coordinator | Module registry | Select all structural modules matching the file's MIME type via prefix matching. Fan-out: every matching module executes. An empty registry produces no structural output — semantic extraction receives no structural context but still runs (default chunking strategy, process whole file). |
| **merge structural output** | Extraction coordinator | StructuralOutput | Combine the outputs of all structural modules for a single file into a unified `StructuralOutput`. Vocabulary terms are unioned (deduplicated). Structural metadata is concatenated. Emissions are kept per-module. The merged output feeds `SemanticInput` for semantic extraction — but an empty merged output is normal and semantic extraction handles it gracefully. |
| **retract contributions** | Engine | Context | Remove a named adapter/enrichment's contribution slot from every edge in the context. An administrative operation exposed via `retract_contributions(context_id, adapter_id)`, not through `ingest()`. After removal, recomputes raw weights and prunes zero-evidence edges. Fires `ContributionsRetracted` and `EdgesRemoved` events. See ADR-027. |
...
### Behavioral
- **Input routing** fan-outs to all **adapters** matching **input kind** (via `classify_input` in `IngestPipeline`)
...
- **ContentAdapter** produces provenance alongside semantics: a **chain** node (**deterministic chain ID** per adapter+source), a **mark** node (carrying annotation text, source file, and tags), and a `contains` edge (chain → mark). All in the **provenance** dimension.
- *(TagConceptBridger was fully removed — tag bridging is domain-specific. Domains needing this behavior implement their own adapter.)*
...
- One **ContentAdapter** type, instantiated with different **adapter IDs** per evidence source (manual, LLM, OCR) — same `process()` logic, distinct **contributions** and **provenance** chains
...
- **Extraction coordinator** handles `extract-file` input kind; runs **registration** blocking, spawns **structural analysis** and **semantic extraction** as sequential background `ingest()` calls
...
- **Module registry** on the **extraction coordinator** holds **structural modules** indexed by MIME type affinity
- **MIME dispatch** selects matching **structural modules** for the file's MIME type — fan-out, not find-first
- Each **structural module** produces a **structural output** and emits graph mutations with its own **adapter ID**
- **Structural outputs** from multiple modules are merged — vocabulary unioned, structural metadata concatenated, emissions kept per-module
- Merged **structural output** feeds forward as **vocabulary bootstrap** and optional structural metadata in `SemanticInput` for **semantic extraction** — semantic extraction always chunks (default strategy works without any structural context) and uses structural metadata to improve chunk boundaries when available
- **Structural analysis** and **semantic extraction** use **independent accumulation** — separate adapter IDs, no sequential priming (Essay 25)
...
- **Semantic extraction** delegates to llm-orc (external service) for LLM calls; **registration** and **structural analysis** are Rust-native
- When llm-orc is unavailable, registration and structural analysis complete normally and semantic extraction is skipped — graceful degradation
...
- **Latent evidence** enters via two paths: (1) **core enrichment** — `EmbeddingSimilarityEnrichment` fires reactively in the enrichment loop when `NodesAdded` events arrive, embedding new nodes and emitting `similar_to` edges above a similarity threshold (ADR-026); (2) **external enrichment** — batch-computed embeddings via llm-orc ensemble, results re-enter via `ingest()` (ADR-026)
...
| 2 | 2026-02-17 | Core enrichment definition | **Expanded:** "Fast (microseconds)" → "Fast (microseconds to low milliseconds)". Added fifth core enrichment: EmbeddingSimilarityEnrichment (ADR-026). Embedding's per-node cost (~15-100ms) is mitigated by batch ONNX inference; similarity computation over cached vectors remains microsecond-scale. | ADR-024 originally defined core enrichments as "fast (microseconds)". ADR-026 expands this. Domain model "Evidence layers and discovery" section updated to acknowledge both core and external enrichment paths for latent evidence. OQ-14 sub-questions marked resolved. |
| 3 | 2026-03-04 | Adapter snapshot, Input router | **Removed:** Adapter snapshot (never implemented, design deferred indefinitely). **Renamed:** "Input router" → "Input routing" — the `InputRouter` struct is removed; routing is now inline in `IngestPipeline` via the `classify_input` free function. See ADR-029 (architectural consolidation). | Actions table updated: "route" actor changed from "Input router" to "IngestPipeline". Behavioral relationship updated. |
| 4 | 2026-03-18 | 55 (structural module routing) | **Softened:** from "structural modules are NOT adapters" to "structural modules use MIME-based dispatch — whether the trait extends Adapter is a BUILD-time decision." The architectural constraint is the routing mechanism (MIME dispatch vs input-kind routing), not whether one trait extends another. | ADR-030 updated: removed "not an adapter" language, added routing distinction as design choice. Structural module concept definition updated. Scenario "Structural module is not an adapter" rewritten as "uses MIME dispatch, not input-kind routing." |
...
> **Resolved:** ADR-026 (embedding as enrichment) and ADR-027 (contribution retraction) address embedding production, discovery gap computation, and embedding drift. Essay 20 provides full research context. Cross-modal embeddings remain a future research direction.

Latent evidence (embedding-derived similarity) enters the graph via two paths: (1) `EmbeddingSimilarityEnrichment` as a core enrichment (reactive, fastembed-rs, ADR-026), and (2) external enrichment via llm-orc ensemble (batch, ADR-026). The discovery gap between structural and latent evidence is informative because the two layers are independently sourced. Resolved sub-questions:

- **Embedding production.** ~~Resolved (ADR-026):~~ Core enrichment via fastembed-rs (nomic-embed-text-v1.5) for reactive use; external enrichment via llm-orc for batch/experimentation. Enrichment ID encodes model name (ADR-027 convention).
- **Discovery gap computation.** ~~Resolved (ADR-024, ADR-026):~~ Reactive core enrichment is sufficient. DiscoveryGapEnrichment reacts to `similar_to` edges from the embedding enrichment.
- **Embedding drift.** ~~Resolved (ADR-027):~~ New model version gets a new enrichment ID (`embedding:nomic-embed-text-v2.0`). Old model's contributions retracted via `retract_contributions()`. Contribution tracking handles the lifecycle: add → update → retract.
