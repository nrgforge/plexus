...
This essay investigates three remaining gaps from Essay 24's extraction research — 31-35% relationship recall, 62% domain-specific entity recall, and the impracticality of the only model that demonstrated strong relationship recall (Qwen3:14b at 87% memory use) — and reports interventions that close all three without larger models or new engine machinery. The central finding is that JSON schema constraint (structured output via Ollama's format parameter) is the dominant intervention for relationship extraction: Mistral:7b with structured output matches Qwen3:14b's recall in one-third the time, because grammar-level constraints eliminate wasted inference on thinking tokens and prose preamble. For entity extraction on domain-specific text, glossary priming normalizes vocabulary toward domain terms while cross-condition union (three primed runs plus one unprimed run) reaches ~85% recall by combining each approach's complementary strengths. The research also establishes that Phase 2 (SpaCy-based co-occurrence and dependency parsing) and Phase 3 (LLM extraction) should operate independently with separate adapter IDs rather than sequentially — priming the LLM with Phase 2's typed relationships caused it to validate rather than extend, reducing recall from 81% to 38%.
...
The Phase 3 extraction ensemble is built and smoke-tested. SpaCy (`en_core_web_trf`) runs as a vocabulary bootstrap — its discovered entities feed the entity-primed agents via a DAG dependency, while relationship, theme, and entity-unprimed agents remain fully independent. A live run on domain text confirmed the architecture: SpaCy found 90+ entities including compound domain noun phrases, the DAG delivered vocabulary to primed agents, run-to-run variation across relationship agents produced genuine diversity, and the unprimed entity agent provided independent signal for reinforcement.

The Phase 2 extractors need implementation as Rust-native adapters. Neither requires new engine design — the contribution tracking, scale normalization, enrichment loop, and deterministic concept IDs that the extraction pipeline relies on have been in place since Essay 06 (reinforcement mechanics) and Essay 12 (provenance infrastructure). The extraction layer is composed from infrastructure that was built for other reasons and works here without modification.

The remaining refinements are optimization, not architecture: filtering numeric entities from SpaCy output, tuning entity-primed prompts to extract rather than summarize, and running against the full 24-essay corpus to validate recall estimates beyond the two-essay spikes.
