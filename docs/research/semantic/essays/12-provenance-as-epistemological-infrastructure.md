...
**Provenance output** (new): a chain node (provenance dimension, deterministic ID per adapter+source), a mark node (provenance dimension, with annotation text, source file, and tags), and a `contains` edge (chain → mark, within provenance dimension).
...
- 2 `tagged_with` edges (structure → semantic, with Hebbian contributions)
...
- 1 `contains` edge (provenance → provenance)
- 2 `references` edges (provenance → semantic, created by TagConceptBridger)
