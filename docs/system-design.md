...
**Version:** 1.3
**Status:** Current (Retrofit + MCP consumer interaction amendment + Default-install deployment-class amendment)
...
| Embedding backends are first-class per deployment class: in-process (library consumers with `features = ["embeddings"]`) and consumer-declared external enrichment (default binaries) | Quality Attribute (Deployment fit) | ADR-026; ADR-038 |
| Default binary is lean — enrichments without an active producer are graceful-idle, not errors | Constraint | ADR-038; ADR-040 |
...
**Owns:** IngestPipeline (adapter vector and enrichment registry behind interior mutability for runtime registration — ADR-037 §5; specific mechanism a BUILD decision), PipelineBuilder (`with_structural_module()`, `with_persisted_specs()`, `default_pipeline()`), classify_input, ClassifyError, input routing logic
...
- `DeclarativeAdapter` — interprets consumer-authored YAML specs delivered programmatically via `PlexusApi::load_spec` (ADR-037). The consumer defines both the extractor logic (llm-orc ensemble or script) and the graph mapping (adapter spec primitives). Plexus provides the interpreter; the consumer provides the spec. This is the extension point for domain-specific extraction without writing Rust.
...
### Embedding Backend Deployment Classes (ADR-038)

`EmbeddingSimilarityEnrichment` (ADR-026) produces `similar_to` edges. Two backends are first-class, selected by deployment class rather than by preference:

**In-process backend — `features = ["embeddings"]`.** Library consumers linking Plexus into their own application where end-users cannot be expected to install llm-orc. Build with `plexus = { version = "X", features = ["embeddings"] }`. `PipelineBuilder::with_default_enrichments()` registers `EmbeddingSimilarityEnrichment` backed by `FastEmbedEmbedder` (fastembed-rs + ONNX Runtime + sqlite-vec). Reactive, in the enrichment loop, in-process.

**External backend — default binary (Homebrew / `cargo-dist` / shell-installer).** The distributed binary ships with `default = []` — no ONNX Runtime weight, no bundled model. `EmbeddingSimilarityEnrichment` is **not registered** by `with_default_enrichments()` in this build. Consumers who want embedding-based discovery activate it by authoring (or adopting) a declarative adapter spec that declares an llm-orc ensemble emitting `similar_to` via `ingest()`. This is the already-sanctioned external-enrichment path (ADR-024, ADR-025) made operational for default-build embedding. Rust core gains no second code path to llm-orc.

**Neither present — graceful-idle baseline.** When the binary is `default = []` and no consumer spec is loaded, CoOccurrence fires on `tagged_with` edges, TemporalProximity fires on nodes carrying `created_at` (after ADR-039's property contract lands), DiscoveryGap is registered but idle (no trigger producer is active — expected per ADR-040), EmbeddingSimilarity is absent entirely. The graph accepts writes, emits events, serves queries. This is the honest lean baseline; onboarding documentation names it as such.

The two backends are peers — different deployment shapes select different mechanisms. Neither is a fallback for the other. Consumer-declared activation is not "feature-gated off embedding elsewhere;" it is the distribution-layer counterpart to the engine-architecture-layer `features = ["embeddings"]` choice.
...
**Fires on:** `EdgesAdded`
**Trigger-source contract (ADR-040):** Source-agnostic — any producer emitting the configured trigger relationship activates the enrichment, via the enrichment loop's re-entry semantics (Invariant 49). Accepted producers for the default parameterization (`trigger = "similar_to"`): in-process `EmbeddingSimilarityEnrichment` under `features = ["embeddings"]`; consumer-declared external enrichment via adapter spec (ADR-024, ADR-038); consumer `create_edge` primitives emitting `similar_to` directly; another consumer's lens translation. Absence of any producer is **graceful-idle by design**, not a defect — DiscoveryGap stays quiet until a trigger is active. The enrichment does not broaden its algorithm to include structural-absence patterns that don't require `similar_to`; those patterns extend through additional `DiscoveryGapEnrichment` instances under different parameterizations declared in a spec's `enrichments:` section.
...
**Backends per deployment class (ADR-038):** In-process — `FastEmbedEmbedder` + sqlite-vec under `features = ["embeddings"]` (library consumers). External — consumer-declared llm-orc ensemble via adapter spec, emitting `similar_to` edges through `ingest()` (default binary; see "Embedding Backend Deployment Classes" subsection above). Test/fallback: `InMemoryVectorStore`.
**Fires on:** `NodesAdded`
...
| ~~register_specs_from_dir full-spec wiring (extract enrichments + lens, not just adapter)~~ — **superseded 2026-04-14**: file-based path removed entirely rather than fixed. ADR-037 §4 superseded. | adapter/pipeline | ADR-037 §4 (superseded) |
...
3. **Transport wiring shell:** mcp imports api for routing, adapter for pipeline construction (PipelineBuilder, classify_input), and graph/storage for initialization. Pipeline construction happens inside `PlexusMcpServer::new()`.
...
| Default binary does not break without llm-orc or consumer-authored spec | Acceptance tests under `default = []` with no llm-orc and no `load_spec` call: ingest succeeds, CoOccurrence fires on tagged content, queries return expected results, DiscoveryGap and EmbeddingSimilarity produce no edges (not errors) | 100% pass under the default feature set | Invariant 49; ADR-038 honest-to-demo baseline |
...
|---|------|-------------|---------|------------|--------|
| 10 | 2026-07-08 | Post-M2/M3 feature batch (issues #4-#11), documented together: (1) `WeightSpec` — `create_edge.weight` accepts a template expression resolved per emission; non-finite resolution warns and degrades to 1.0 (`75bea80`, issue #7). (2) `TranslationRule.min_corroboration` — a merged lens pair emits only when >= N distinct from-relationships evidence it; all-below-threshold rounds are quiescence (`081b8ac`, issue #4). (3) Background phases run the enrichment loop: `run_background_enrichment` in the ExtractionCoordinator uses the pipeline's shared registry cell, closing the T11 lens gap (`1706096`, issue #5). (4) `TemporalProximityEnrichment.with_node_types` + declarative `node_types:` param; the DEFAULT enrichment is fragment-scoped as of v0.4.0 — behavior change, enrichment ID changed so prior contributions orphan per Invariant 13 (`a60dcbf`, `c9eefcc`, issue #6). (5) `sync_spec_lenses` is bidirectional: vanished spec rows deregister their lens unless another examined row references the same lens id — closes Amendment 9's residual (`16e18a4`, issue #11). (6) SemanticAdapter `build_input` includes document `content` (capped 24k chars, graceful omit per Invariant 47) — extraction agents previously received only metadata (`95839fb`, issue #3). No module boundaries changed. | Post-graduation engineering queue (GitHub issues #2-#11), each validated end-to-end via the play harness against real Ollama/MCP surfaces. | issues #3-#7, #9, #11; M3 probe findings (#12-#14 filed for future work) | Current |
| 9 | 2026-07-07 | Cross-process lens reactivity (Invariant 62, issue #10): `IngestPipeline::sync_spec_lenses` syncs lens enrichments from the context's specs table at both ingest entry points, keyed by `(context_id, adapter_id, loaded_at)` so unchanged rows are not re-parsed per ingest. Lens-only, mirroring rehydration semantics — adapter wiring stays transient to the loading consumer's process. Invariant 62's "reactive to all future emissions by any consumer" now holds per-context rather than per-process. Residual: sync is additive-only; `unload_spec` does not propagate to other live processes (issue #11). No module boundaries changed. | Harness `latent` scenario: carrel lens coverage over a sweep performed in another process was 0 until a spec-holding process ingested; post-fix immediate (asserted). | Invariant 62; issue #10; commit `83922e1` | Current |
| 8 | 2026-07-07 | ADR-017 §2 conformance: `PlexusEngine::reload_if_changed()` (data_version check + cache reload) existed but was never called from consumer-facing paths, leaving long-lived multi-process readers permanently stale. Wired into `PlexusApi::resolve` (every read path resolves through it), `context_list`'s unfiltered branch, and a new `resolve_for_ingest` shared by `ingest`/`ingest_with_adapter` so the enrichment loop computes over fresh state. No module boundaries changed; no new dependency edges. Write-side was already safe (incremental upsert, ADR-017 §3). | M0 consistency matrix (`tools/play-harness/play.py matrix`, issue #1): concurrent topology fully STALE on cache-backed reads while `changes_since` was LIVE (store-direct). Post-fix: all 18 matrix cells LIVE. | ADR-017 §2; issue #1; commit `d97e5de` | Current |
...
| 6 | 2026-04-14 | MCP cycle WP-H.1 — scope-reductive design correction. Removed file-based spec auto-discovery entirely: `IngestPipeline::register_specs_from_dir`, `PipelineBuilder::with_adapter_specs`, and the `adapter-specs/` call in `default_pipeline` are deleted. `PlexusMcpServer::with_project_dir` is also removed — its sole purpose was to thread a project directory into `with_adapter_specs`; hosts call `PlexusMcpServer::new(engine)` directly. `PipelineBuilder::default_pipeline(engine)` drops its `project_dir` parameter. The Spec source precedence list narrows from three entries to two (code-registered + DB-persisted). ADR-037 §4 marked superseded; invariant 61 narrowed (see domain-model Amendment 8). | Standing principle check during WP-G.2 reflection: file-based auto-discovery violates Invariant 61 (consumer owns spec) because directory scanning at construction time registers specs on the consumer's behalf rather than on an intentional `load_spec` call. It also creates latent double-registration when the same `adapter_id` is present in both `adapter-specs/` and the persisted `specs` table. | ADR-037 §4 (superseded); Invariant 61 (narrowed); domain-model Amendment 8 (2026-04-14); product-discovery rows on deployment-time and auto-wiring (revised) | Current |
| 7 | 2026-04-22 | Default-install experience + lens design principles cycle — ARCHITECT (light-touch). (1) Architectural Drivers: retired single "Embedding support behind feature flag" driver; added two drivers — "Embedding backends are first-class per deployment class (in-process + consumer-declared external)" citing ADR-026 and ADR-038, and "Default binary is lean — graceful-idle for enrichments without an active producer" citing ADR-038 and ADR-040. (2) New "Embedding Backend Deployment Classes" subsection in Enrichment Architecture names the three deployment shapes (in-process `features=["embeddings"]`, default binary with consumer-declared external enrichment, neither-present graceful-idle baseline) and states that Rust core gains no second code path to llm-orc. (3) DiscoveryGapEnrichment entry in Core Enrichment Algorithms gains an explicit trigger-source contract paragraph (source-agnostic acceptance; accepted producer list; graceful-idle by design; no algorithm broadening). (4) EmbeddingSimilarityEnrichment "Backends" line expanded from feature-flag-only framing to per-deployment-class framing citing ADR-038. (5) New fitness criterion: "Default binary does not break without llm-orc or consumer-authored spec" — verified by acceptance tests under `default = []`. **No module boundaries changed, no new dependency edges, no responsibility matrix changes.** ADR-040's DiscoveryGap trigger-coupling story confirmed as naming/documentation-level only — the enrichment's existing parameterization and the enrichment loop's re-entry semantics (Invariant 49) already support the source-agnostic contract; the amendment surfaces what was structurally true and makes it visible. | ADRs 038–042 landed in DECIDE (2026-04-21); BUILD documentation deliverables (README + worked-example spec + onboarding material) are load-bearing for ADR-038's reframing per cycle-status feed-forward. | ADR-038, ADR-039, ADR-040, ADR-041, ADR-042; cycle-status §Context for Resumption; DECIDE gate reflection note | Current |
| 5 | 2026-04-07 | MCP consumer interaction surface cycle. (1) `api` gains `load_spec`/`unload_spec` methods and `SpecLoadResult`/`SpecLoadError`/`SpecUnloadError` types — transport-independent, fail-fast, all-or-nothing. (2) `storage` gains `specs` table (composite PK on `(context_id, adapter_id)`) and `persist_spec`/`query_specs_for_context`/`delete_spec` methods on `GraphStore` trait (default no-ops for non-SQLite backends). (3) `adapter/pipeline` gains interior mutability on the adapter vector and enrichment registry for runtime registration; ~~`register_specs_from_dir` bug fix (extracts enrichments + lens, not just adapter)~~ *(superseded 2026-04-14 — WP-H.1 removed file-based auto-discovery entirely, see Amendment 6)*; `PipelineBuilder::with_persisted_specs(Vec<PersistedSpec>)` rehydrates persisted lens enrichments at construction time (enrichment-only, skips effect a and adapter wiring). (4) `mcp` gains 7 new `#[tool]` handlers (`load_spec`, `find_nodes`, `traverse`, `find_path`, `changes_since`, `list_tags`, `shared_concepts`), bringing tool count from 9 to 16; all use flat optional parameter fields. (5) `api` + `query` + `mcp` — `evidence_trail` gains optional `QueryFilter` parameter piped through `StepQuery`, closing the last Invariant 59 gap. (6) `query` gains `RankBy::NormalizedWeight(Box<dyn NormalizationStrategy>)` variant wired to existing normalization strategies, closing ADR-034 conformance debt. (7) Architectural drivers expanded for Invariants 60–62. (8) Fitness criteria added: spec validation gates graph work, consumer ownership of spec, durable vocabulary, durable enrichment registration, fail-fast atomicity, interior mutability scope, MCP tool count = 16. (9) 7 new boundary integration tests and 4 new invariant enforcement tests. **No new module-level dependency edges** — all new work flows through existing seams (`api → graph → storage`, `api → adapter/pipeline`, `mcp → api`, host orchestrates rehydration at builder time). The specs table is the context's lens registry; any library instance against that context transiently runs those lenses on behalf of the context, not on behalf of the originally-loading consumer. | MCP consumer interaction surface RDD cycle: DISCOVER (update), MODEL (amendment), DECIDE (ADR-036, ADR-037), audits complete. Triggered by Reflection 003 surfacing the single-consumer assumption in the query surface cycle and product discovery (2026-04-02) formalizing multi-consumer interaction model with fail-fast validation as the e2e acceptance criterion. | ADR-036 (MCP query surface), ADR-037 (consumer spec loading); Invariants 60–62; Reflection 003 (multi-consumer lens interaction); product-discovery.md (2026-04-02); domain-model.md amendment 7 (2026-04-02) | Current |
