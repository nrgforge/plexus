#!/bin/bash
# Spike Q2: Domain vocabulary priming for entity extraction
# 2x2 design: (unprimed vs glossary-primed) x (unconstrained vs structured output)
# Model: Mistral:7b on Essay 12 (domain-specific text, worst entity recall)

set -e
RESULTS_DIR="$(dirname "$0")/results"
mkdir -p "$RESULTS_DIR"

# Essay 12 text (stripped of markdown formatting for cleaner input)
...
# Base entity extraction prompt (from Essay 24)
...
# Glossary from the domain model (term names only, ~200 tokens)
...
run_condition() {
    ...
    echo "=== Running condition: $name ==="
    echo "  Format: $use_format"
    ...
    EVAL_S=$(echo "scale=1; $EVAL_NS / 1000000000" | bc)

    if [ "$EVAL_COUNT" -gt 0 ]; then
        CHARS_PER_TOK=$(echo "scale=2; $CONTENT_LEN / $EVAL_COUNT" | bc)
    else
        CHARS_PER_TOK="N/A"
    fi

    echo "  Total: ${TOTAL_S}s | Gen: ${EVAL_S}s | Prompt tokens: $PROMPT_EVAL_COUNT | Gen tokens: $EVAL_COUNT | Chars/tok: $CHARS_PER_TOK"
    echo "  Content length: $CONTENT_LEN chars"
    echo ""
}

echo "========================================="
echo "Q2 Spike: Entity Extraction 2x2 Matrix"
echo "Model: $MODEL | Essay: 12 (Provenance)"
echo "========================================="
echo ""

# Condition A: Unprimed + Unconstrained (Essay 24 baseline replication)
run_condition "A-unprimed-unconstrained" "$BASE_PROMPT" "false"

# Condition B: Unprimed + Structured output
run_condition "B-unprimed-structured" "$BASE_PROMPT" "true"

# Condition C: Glossary-primed + Unconstrained
PRIMED_PROMPT="$GLOSSARY

$BASE_PROMPT"
run_condition "C-primed-unconstrained" "$PRIMED_PROMPT" "false"

# Condition D: Glossary-primed + Structured output
run_condition "D-primed-structured" "$PRIMED_PROMPT" "true"

echo "========================================="
echo "All conditions complete. Results in $RESULTS_DIR/"
echo "========================================="
