#!/bin/bash
# Q2 Union experiment: multi-run entity extraction
# Tests: 3x primed+structured (same-condition union) + cross-condition union with unprimed
set -e
...
BASE_PROMPT='Extract ONLY the key entities and concepts from the provided text.
...
run_structured_primed() {
    local name="$1"
    local temp="$2"

    PAYLOAD=$(jq -n \
        --arg model "$MODEL" \
        --arg system "$PRIMED_PROMPT" \
        --arg user "$ESSAY_TEXT" \
    ...
    echo "$RESPONSE" > "$RESULTS_DIR/$name.json"
    ...
    LABELS=$(echo "$RESPONSE" | jq -r '.message.content' | jq -r '.concepts[].label' | sort)
    COUNT=$(echo "$LABELS" | wc -l | tr -d ' ')

    echo "=== $name === (${TOTAL_S}s, $COUNT entities, $EVAL_COUNT tokens)"
    echo "$LABELS"
    echo ""
}
...
run_unprimed_free() {
    ...
    LABELS=$(echo "$CONTENT" | jq -r '.concepts[].label' 2>/dev/null | sort || echo "(raw output)")
    COUNT=$(echo "$LABELS" | wc -l | tr -d ' ')
...
echo "==========================================="
echo "Q2: Multi-Run Entity Union Experiment"
echo "Model: $MODEL | Essay: 12"
echo "==========================================="
echo ""

# 3x primed+structured at temp 0.3 (higher temp for variation)
echo "--- 3x Primed+Structured (temp 0.3) ---"
run_structured_primed "union-D-run1" 0.3
run_structured_primed "union-D-run2" 0.3
run_structured_primed "union-D-run3" 0.3

# 1x unprimed+free for cross-condition union
echo "--- 1x Unprimed+Free (temp 0.15, for cross-condition union) ---"
run_unprimed_free "union-A-run1"

echo "==========================================="
echo "Computing union statistics..."
echo "==========================================="

# Extract all labels and compute union/intersection
echo ""
echo "--- Per-run labels ---"
for i in 1 2 3; do
    echo "D-run$i:"
    jq -r '.message.content' "$RESULTS_DIR/union-D-run$i.json" | jq -r '.concepts[].label' | sort
    echo ""
done

echo "A-run1:"
jq -r '.message.content' "$RESULTS_DIR/union-A-run1.json" | jq -r '.concepts[].label' 2>/dev/null | sort
echo ""

# Compute union across D runs
echo "--- D-only union (3 runs) ---"
D_UNION=$(for i in 1 2 3; do
    jq -r '.message.content' "$RESULTS_DIR/union-D-run$i.json" | jq -r '.concepts[].label'
done | sort -u)
echo "$D_UNION"
D_COUNT=$(echo "$D_UNION" | wc -l | tr -d ' ')
echo "Total unique: $D_COUNT"
echo ""

# Compute intersection across D runs
echo "--- D-only intersection (all 3 runs) ---"
D_INTERSECT=$(comm -12 \
    <(comm -12 \
        <(jq -r '.message.content' "$RESULTS_DIR/union-D-run1.json" | jq -r '.concepts[].label' | sort) \
        <(jq -r '.message.content' "$RESULTS_DIR/union-D-run2.json" | jq -r '.concepts[].label' | sort)) \
    <(jq -r '.message.content' "$RESULTS_DIR/union-D-run3.json" | jq -r '.concepts[].label' | sort))
echo "$D_INTERSECT"
D_INT_COUNT=$(echo "$D_INTERSECT" | wc -l | tr -d ' ')
echo "Stable core: $D_INT_COUNT"
echo ""

# Compute full union (D + A)
echo "--- Full union (3x D + 1x A) ---"
FULL_UNION=$(
    for i in 1 2 3; do
        jq -r '.message.content' "$RESULTS_DIR/union-D-run$i.json" | jq -r '.concepts[].label'
    done
    jq -r '.message.content' "$RESULTS_DIR/union-A-run1.json" | jq -r '.concepts[].label' 2>/dev/null
)
FULL_UNIQUE=$(echo "$FULL_UNION" | sort -u)
echo "$FULL_UNIQUE"
FULL_COUNT=$(echo "$FULL_UNIQUE" | wc -l | tr -d ' ')
echo "Total unique: $FULL_COUNT"
