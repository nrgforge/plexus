//! Co-occurrence detection (ADR-004, ADR-010)
//!
...
//!
//! Two implementations:
//! - `CoOccurrenceEnrichment` — the enrichment form (ADR-010), idempotent, for
//!   use in the enrichment loop.
//! - `CoOccurrenceAdapter` — the legacy adapter form (ADR-004), for backward
//!   compatibility with direct-invocation tests.
...
// ── CoOccurrenceEnrichment (ADR-010) ────────────────────────────────
...
// ── CoOccurrenceAdapter (legacy, ADR-004) ───────────────────────────
...
mod tests {
    ...
    // ── CoOccurrenceEnrichment tests ────────────────────────────────

    /// Helper: build a Context with fragments and concepts via FragmentAdapter,
    /// returning the context directly (not Arc<Mutex<>>).
    async fn build_enrichment_context(
        fragments: Vec<(&str, Vec<&str>)>,
    ) -> Context {
    ...
    #[tokio::test]
    async fn enrichment_detects_cooccurrence() {
        let ctx = build_enrichment_context(vec![
            ("F1", vec!["travel", "avignon"]),
        ]).await;
    ...
    #[tokio::test]
    async fn enrichment_idempotent_skips_existing_edges() {
        let mut ctx = build_enrichment_context(vec![
            ("F1", vec!["travel", "avignon"]),
        ]).await;

        // Pre-add the may_be_related edges
        let travel_id = NodeId::from_string("concept:travel");
        let avignon_id = NodeId::from_string("concept:avignon");
        ...
        // Should be quiescent — edges already exist
        assert!(enrichment.enrich(&events, &ctx).is_none());
    }

    #[tokio::test]
    async fn enrichment_quiescent_on_non_structural_events() {
        let ctx = build_enrichment_context(vec![
            ("F1", vec!["travel", "avignon"]),
        ]).await;
        ...
        // NodesRemoved is not a structural event that affects co-occurrence
    ...
    #[tokio::test]
    async fn enrichment_normalized_scores() {
        let ctx = build_enrichment_context(vec![
            ("F1", vec!["travel", "avignon"]),
            ("F2", vec!["travel", "avignon", "paris"]),
        ]).await;
    ...
    #[tokio::test]
    async fn enrichment_empty_graph_returns_none() {
        let ctx = Context::new("test");
        let enrichment = CoOccurrenceEnrichment::new();
        let events = vec![nodes_added_event(vec!["n1"])];
