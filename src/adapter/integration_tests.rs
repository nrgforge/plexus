//! Cross-module integration tests for the adapter layer.
//!
//! Pruned 2026-07-08 against the 2026-03-18 audit: redundant tests (contracts
//! covered by `tests/acceptance/`) were deleted, and single-module tests were
//! relocated to their module `#[cfg(test)]` blocks (`cancel.rs`,
//! `sink/engine_sink.rs`, `enrichment/enrichment_loop.rs`, `traits.rs`).
//!
//! What remains here is deliberately cross-module: pipeline + enrichment loop
//! interplay, spike scenarios at scale, embedding enrichment through the full
//! pipeline, normalization through persistence, layered provenance, declarative
//! spec registration, and JSON wire-format routing. Several are still candidates
//! for migration to `tests/acceptance/` in a future cycle.
//!
//! The canonical contract test suite is `tests/acceptance/`.
...
mod tests {
    ...
    // ================================================================
    // Mock enrichments shared by the pipeline integration tests below.
    // (Loop-level scenarios live in enrichment/enrichment_loop.rs.)
    // ================================================================
