...
impl<'a> ProvenanceApi<'a> {
    ...
    pub fn add_mark(
        ...
        // ADR-009: Tag-to-concept bridging
        if let Some(ref tag_list) = tags {
            let context = self.engine.get_context(&self.context_id)
                .ok_or_else(|| PlexusError::ContextNotFound(self.context_id.clone()))?;
            let matching: Vec<NodeId> = tag_list.iter().filter_map(|tag| {
            ...
            for concept_id in matching {
                let ref_edge = Edge::new_cross_dimensional(
                    mark_id.clone(),
                    dimension::PROVENANCE,
                    concept_id,
                    dimension::SEMANTIC,
                ...
                self.engine.add_edge(&self.context_id, ref_edge)?;
            }
        }
...
mod tests {
    ...
    // ================================================================
    // ADR-009: Tag-to-Concept Bridging Tests
    // ================================================================

    /// Helper: create a concept node in a context's semantic dimension
    fn add_concept(engine: &PlexusEngine, ctx_id: &ContextId, name: &str) {
        let mut node = Node::new_in_dimension("concept", ContentType::Concept, dimension::SEMANTIC);
        node.id = NodeId::from_string(format!("concept:{}", name));
        engine.add_node(ctx_id, node).unwrap();
    }

    // === Scenario: Mark with matching concept gets references edge ===
    #[test]
    fn mark_with_matching_concept_gets_references_edge() {
        let (engine, ctx_id) = setup_engine_with_context("provence-research");
        add_concept(&engine, &ctx_id, "travel");

        let api = ProvenanceApi::new(&engine, ctx_id.clone());
        let chain_id = api.create_chain("reading-notes", None).unwrap();
        let mark_id = api.add_mark(
            &chain_id, "notes.md", 1, "travel note",
            None, None, Some(vec!["#travel".into()]),
        ).unwrap();

        let ctx = engine.get_context(&ctx_id).unwrap();
        let refs: Vec<_> = ctx.edges().filter(|e| {
            e.source == NodeId::from(mark_id.as_str())
                && e.relationship == "references"
        }).collect();
        assert_eq!(refs.len(), 1);
        assert_eq!(refs[0].target, NodeId::from_string("concept:travel"));
    ...
    #[test]
    fn mark_with_multiple_matching_concepts() {
        let (engine, ctx_id) = setup_engine_with_context("provence-research");
        add_concept(&engine, &ctx_id, "travel");
        add_concept(&engine, &ctx_id, "avignon");
        ...
        let targets: HashSet<String> = refs.iter().map(|e| e.target.to_string()).collect();
    ...
    #[test]
    fn mark_with_nonmatching_tag_no_references() {
        let (engine, ctx_id) = setup_engine_with_context("provence-research");
        add_concept(&engine, &ctx_id, "travel");
        ...
        let chain_id = api.create_chain("reading-notes", None).unwrap();
        let mark_id = api.add_mark(
            &chain_id, "notes.md", 1, "walking note",
            None, None, Some(vec!["#walking".into()]),
        ).unwrap();

        // Mark created successfully
        let ctx = engine.get_context(&ctx_id).unwrap();
        assert!(ctx.get_node(&NodeId::from(mark_id.as_str())).is_some());
    ...
    // === Scenario: Mark created before concept is not bridged ===
    #[test]
    fn mark_before_concept_not_bridged() {
        let (engine, ctx_id) = setup_engine_with_context("provence-research");
        ...
        // Mark created, no references
        let ctx = engine.get_context(&ctx_id).unwrap();
        assert!(ctx.get_node(&NodeId::from(mark_id.as_str())).is_some());
        let refs: Vec<_> = ctx.edges().filter(|e| e.relationship == "references").collect();
        assert_eq!(refs.len(), 0);

        // Later add concept — no retroactive bridging
        drop(ctx);
        add_concept(&engine, &ctx_id, "travel");
        let ctx = engine.get_context(&ctx_id).unwrap();
        let refs: Vec<_> = ctx.edges().filter(|e| e.relationship == "references").collect();
    ...
    #[test]
    fn mark_with_no_tags_no_references() {
        let (engine, ctx_id) = setup_engine_with_context("provence-research");
        add_concept(&engine, &ctx_id, "travel");
        ...
        let chain_id = api.create_chain("reading-notes", None).unwrap();
        let mark_id = api.add_mark(
            &chain_id, "notes.md", 1, "plain note",
            None, None, None,
        ).unwrap();
